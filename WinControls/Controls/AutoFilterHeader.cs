...
{
    public class AutoFilterHeader : DataGridViewColumnHeaderCell
    {

        public void PerformSearchInvoke(string strSearchParam, int _ColumnIndex)
        {
            //_popUp.PerformSearchInvoke(strSearchParam, parentGrid);
            _popUpText.SearchText = strSearchParam;
            SearchText = strSearchParam;
            _popUpText.FilterData(strSearchParam, true, this.DataGridView,_ColumnIndex);
            SearchText = _popUpText.SearchText;
            if (SearchText != "")
                filtered = true;
            else
                filtered = false;
        ...
        private Image _imgSearchActive;
        private Image _imgSearchMouseIn;
        /// <summary>
        ...
        {
            if (tsRemoveFilter.Enabled)
                RemoveFilter();
        }

        public  void RemoveFilter()
        {
            _popUpText.RemoveFilter();
            //_popUpText.DrawText();
            _popUp.Visible = false;
            //_popUpText.OnFilterRemoved(this.DataGridView, this.OwningColumn.Index, -1, _popUp);
            _popUpText.ShowSearch(this.DataGridView, this.OwningColumn.Index, -1, _popUp,false);
            _popUp.Hide();
            //ReadnSearchGrid dgView = (ReadnSearchGrid)this.DataGridView;
            //foreach (DataGridViewColumn col in dgView.Columns)
            //{
            //    if (col.HeaderText.ToUpper().StartsWith("MASK_"))
            //        continue;
            //    int CurrentColumnIndex = -1;
            //    CurrentColumnIndex = this.OwningColumn.Index;
            //    if (col.Index != CurrentColumnIndex)
            //        continue;
            //    AutoFilterHeader filter = (AutoFilterHeader)col.HeaderCell;
            //    filter._popUpText.SearchText = "";
            //    filter._popUpText.txtSearch.Text = "";

            //}
            //_popUpText.txtSearch.Text = String.Empty;
            filtered = false;
            //_popUpText.SearchText = "";
            ContextMenuChangeState(filtered);
        }
        ...
        private void ContextMenuChangeState(bool _Enabled)
        {
            this.tsRemoveFilter.Enabled = _Enabled;
        }
        ...
        private void _popUp_LostFocus(object sender, EventArgs e)
        {
            SearchText = _popUpText.SearchText;
            if (SearchText != "")
                filtered = true;
        ...
        #region Search Popup Declarations

        public ReadnSearchPopUp PopUpText
        {
            get { return _popUpText; }
        }

        private ReadnSearchPopUp _popUpText;
        ...
        /// class. 
        /// </summary>
        public AutoFilterHeader()
        {
            Assembly Asm = Assembly.GetExecutingAssembly();
            string Resource_name1 = "WinForms.Resources.m_zoom_16x16.jpg";
            Stream stream1 = Asm.GetManifestResourceStream(Resource_name1);
            _imgSearch = Image.FromStream(stream1);
        ...
        {
            return new AutoFilterHeader(this);
        }
            ...
            DataGridViewPaintParts paintParts)
        {
            ...
            if (_MouseIn)
            {
                graphics.DrawImage(_imgSearchMouseIn, buttonBounds);
            }
            else
            {
                if (filtered)
                {
                    //Search Active
                    graphics.DrawImage(_imgSearchActive, buttonBounds);
                }
                else
                {
                    //Search Not Active
                    graphics.DrawImage(_imgSearch, buttonBounds);
                }
            }
        ...
        protected override void OnMouseDown(DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                OpenOnRight = true;
                //this.DataGridView.FirstDisplayedScrollingColumnIndex = 0;
                //return;
            }
                ...
                e.X + cellBounds.Left - scrollingOffset, e.Y + cellBounds.Top))
            {
                ...
                if (e.Button == MouseButtons.Left)
                    ShowDropDownList(e.RowIndex, e.ColumnIndex, OpenOnRight);
        ...
        public System.Windows.Forms.ContextMenuStrip Cntxtmenu;
        public ToolStripMenuItem tsRemoveFilter;
        ...
        private void SortByColumn()
        {
            ...
            if (((ReadnSearchGrid)this.DataGridView).HandleGridSort)
                ((ReadnSearchGrid)this.DataGridView).SetPrimaryKeyValues();
            ...
            {
                this.DataGridView.Tag = this.DataGridView.DataSource;   
            }

            DataTable dtTaggedSource = (DataTable)this.DataGridView.Tag;
            int iColIndex = get_actual_colum_index(OwningColumn.Index, dtTaggedSource);
            if (direction == ListSortDirection.Ascending)
                dtTaggedSource.DefaultView.Sort = dtTaggedSource.Columns[iColIndex].ColumnName + " ASC";
            else
                dtTaggedSource.DefaultView.Sort = dtTaggedSource.Columns[iColIndex].ColumnName + " DESC";
            ...
            if (((ReadnSearchGrid)this.DataGridView).HandleGridSort)
            {
                Hashtable iPrimaryKeyColumnIndices;
                Hashtable objHashPrimaryKeyColumnNames;
                iPrimaryKeyColumnIndices = ((ReadnSearchGrid)this.DataGridView).PrimaryKeyColumnIndices;
                objHashPrimaryKeyColumnNames = ((ReadnSearchGrid)this.DataGridView).PrimaryKeyColumnNames;

                DataTable dtSource = null;
                if (this.DataGridView.Tag != null)
                {
                    dtSource = ((DataTable)this.DataGridView.Tag).DefaultView.ToTable();
                }
                else
                {
                    dtSource = (DataTable)this.DataGridView.DataSource;
                }
                ...
                if (strExpression != "" && dtSource != null)
                {
                    if (dtSource.Rows.Count != 0)
                    {
                        ...
                        this.DataGridView.FirstDisplayedScrollingRowIndex = iSelected;
                        ((ReadnSearchGrid)this.DataGridView).Select(iSelected);
                    }
                }
                //**
            }
        ...
        /// </summary>
        public  void ShowDropDownList(int _RowIndex, int _ColumnIndex,bool OpenOnRight)
        {
         //   Debug.Assert(this.DataGridView != null, "DataGridView is null");
           ...
           // this.DataGridView.InvalidateCell(this);
            ...
            _popUpText.ShowSearch(this.DataGridView, _ColumnIndex, _RowIndex, _popUp, OpenOnRight);
            this.DataGridView.InvalidateCell(this);
        }
        ...
        private void UpdateFilter()
        {
            ...
            //    filtered = false;
            //    currentColumnFilter = String.Empty;
            //    RemoveFilter(this.DataGridView);
            //    return;
            //}
            ...
            ////{
            ////    frmCustomFilter _custom = new frmCustomFilter(columnProperty,this.DataGridView);
            ////    _custom.ShowDialog();
            ...
            //        frmCustomFilter _custom = new frmCustomFilter(columnProperty, this.DataGridView);
            //        _custom.ShowDialog();
            //        newColumnFilter = _custom.Filter;
            //        break;
            //    default:
        ...
        public static string selectedFilterValue1;
